...
class Service extends egg.Service {
  ...
  async parse({ directory, data }) {
    ...
    for (const filepath of filepaths) {
      ...
      const comments = commentParser.parse(config.content);
      for (const comment of comments) {
        if (comment.description === 'template config') { // 第一行 为template config的注释作为模板记录
          ...
          for (const tag of comment.tags) {
            if (tag.tag === 'param' && tag.name) {
              ...
              } else if (tag.name === 'copy') { // 只作为警告文件. 不会生成
                config.copy = parseInt(config.data[tag.name]);
              }
            }
          }
        }
